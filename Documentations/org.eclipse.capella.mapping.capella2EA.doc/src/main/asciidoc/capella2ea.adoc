...
*Note:* this chapter describes the check of the installation only for the update site installation case. 
In order to check the dropin installation, check that the Model transformation menu is available as it is described in the <<Open Contextual "Model transformation" menu>> section. 
...
The bridge does not keep the order of the model ekelents in Enterprise Architect as in Capella.
